...
async function main() {
  console.log("Purging all boss battles for user 47 from database...");
  ...
  if (error) {
    console.error("Failed to delete bosses:", error.message);
    return;
  }
  console.log("Purge complete!");

  console.log("Regenerating a single unified boss battle...");
  try {
    const response = await fetch("http://localhost:3000/api/plaid/analyze-transactions", {
      method: "POST",
