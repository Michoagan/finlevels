...
async function main() {
  console.log("Calling sync-and-validate API to catch the 500 error...");
  try {
    const response = await fetch("http://localhost:3000/api/plaid/sync-and-validate", {
      ...
      body: JSON.stringify({
        token: token,
        // sandbox plaid token for user 47 is automatically loaded from DB if clientAccessToken is undefined
      })
    });
